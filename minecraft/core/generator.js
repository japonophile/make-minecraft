...
Blockly.CodeGenerator.prototype.blockToCode = function(block) {
  ...
  /* TODO: HORRIBLE HORRIBLE HACK
     For some reason MC functions are returning arrays but values get converted into strings here */
  if (!(code instanceof Array)) {
  	 var str = code + '';
    if (str[str.length - 2] == ',') { // if string is blabla,4 -> array
    	 code = [str.substring(0, str.length - 2), str[str.length - 1]];
    }
  }
  if (code instanceof Array) {
    // Value blocks return tuples of code and operator order.
